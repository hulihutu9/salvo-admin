...
use crate::model::gen_table_model::{GenTableList, GenTableListPayload, GenTableModifyPayload};
...
pub async fn get_gen_table_page(req:&mut Request)->Res<Page<GenTableList>>{
    ...
    match_ok(gen_table_service::get_gen_table_page(payload.page_num,payload.page_size,
                                                   payload.table_name,payload.table_comment,
                                                   payload.create_time).await)
}
